...
class ProductFactory {
  ...
  static createProduct(type, options) {
    if (type === this.TYPE.BOOK) {
      const { title, author, coverColor } = options;
      const book = new Book(title, author, coverColor);
      return book;
    } else if (type === this.TYPE.AUDIOBOOK) {
      const { title, author, audioLength } = options;
      const audioBook = new AudioBook(title, author, audioLength);
      return audioBook;
    } else {
      throw Error(`Такого типу продукту не існує: ${type}`);
    }
  }
